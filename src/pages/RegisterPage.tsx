...
export function RegisterPage() {
  ...
  const fetchEvent = async (id: string) => {
    try {
      ...
      setEvent(evt);
      setGroupSize(evt.min_team_size || 5);
        ...
        <>
          ...
          {step === 1 && (
            <div className="card">
                ...
                {availableTypes.map(type => (
                  ...
                  >
                    ...
                    <span className="block text-xs text-gray-500">
                      {type === 'solo' ? '1 person' : type === 'duo' ? '2 people' : type === 'trio' ? '3 people' : type === 'quad' ? '4 people' : `${event.min_team_size || 2}–${event.max_team_size} people`}
                    </span>
                  </button>
                ))}
              </div>
              ...
              {regType === 'group' && (() => {
                const minG = event.min_team_size || 2;
                return (
                  <div className="mt-4">
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      Group Size ({minG} to {event.max_team_size})
                    </label>
                    <input
                      type="number"
                      min={minG}
                      ...
                      value={groupSize}
                      onChange={e => setGroupSize(Math.min(event.max_team_size, Math.max(minG, parseInt(e.target.value) || minG)))}
